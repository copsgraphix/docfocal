...
export default function SummarizerPage() {
  ...
  async function handleDownload() {
    ...
    const formData = new FormData();
    formData.append("file", file);
    const res = await fetch("/api/ai/summarize?download=1", { method: "POST", body: formData });
    if (!res.ok) return;
    const blob = await res.blob();
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = file.name.replace(/\.(pdf|docx?)$/i, "") + "-summary.docx";
    a.click();
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mb-6">
        ...
        <h2 className="text-2xl font-bold text-text-primary">PDF/DOCX Summarizer</h2>
        <p className="mt-1 text-sm text-text-secondary">
          Upload a PDF or Word document and AI will generate a structured summary.{" "}
      ...
      >
        ...
        {file ? (
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={(e) => { e.stopPropagation(); setFile(null); setOutput(""); setDone(false); }}
      ...
      {output && (
        <div className="mt-6 rounded-xl border border-border bg-bg-main p-5">
          ...
          <div className="whitespace-pre-wrap text-sm text-text-primary leading-relaxed">
            {output}
            {generating && (
              <span className="ml-1 inline-block h-4 w-0.5 animate-pulse bg-brand-primary" />
            )}
          </div>
        </div>
      )}
